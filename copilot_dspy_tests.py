...
def test_is_token_valid_returns_true_when_no_expires_at():
    manager = CopilotTokenManager(config_dir="/tmp/test-copilot-dspy")
    assert manager._is_token_valid({"access_token": "ghu_abc"}) is True


def test_is_token_valid_returns_true_when_expires_at_unparseable():
    manager = CopilotTokenManager(config_dir="/tmp/test-copilot-dspy")
    assert manager._is_token_valid({"access_token": "ghu_abc", "expires_at": "not-a-date"}) is True


def test_is_token_valid_returns_false_when_expired():
    manager = CopilotTokenManager(config_dir="/tmp/test-copilot-dspy")
...
def test_is_token_valid_returns_true_when_not_expired():
    manager = CopilotTokenManager(config_dir="/tmp/test-copilot-dspy")
